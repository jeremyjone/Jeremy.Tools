...
namespace Jeremy.Tools.Common
{
    ...
    {
        private static readonly Dictionary<string, string> MimeMappingTable;
        private static void AddMimeMapping(string extension, string mimeType)
        {
            MimeMapping.MimeMappingTable.Add(extension, mimeType);
        }

        public static string GetMimeMapping(string fileName)
        {
            var num = fileName.LastIndexOf('.');
            if (0 >= num || num <= fileName.LastIndexOf('\\')) return "application/octet-stream";
            var key = fileName.Substring(num);
            return MimeMapping.MimeMappingTable.ContainsKey(key) ? MimeMapping.MimeMappingTable[key] : "application/octet-stream";
        }
        static MimeMapping()
        {
            MimeMapping.MimeMappingTable = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
